...
JM_fnc_fireRocketsSimple = {
    ...
    // Random approach direction (purely visual)
    private _dir = random 360;
    private _dirVec = [sin _dir, cos _dir, 0];

    for "_i" from 1 to _count do {

        // Impact point with dispersion
        private _impact = _targetASL vectorAdd [
            [-_disp, _disp] call BIS_fnc_randomNum,
            [-_disp, _disp] call BIS_fnc_randomNum,
            0
        ];

        // Spawn position behind + above
        private _spawn = _impact vectorAdd (_dirVec vectorMultiply _offset);
        _spawn set [2, (_impact select 2) + _height];

        private _rocket = _ammo createVehicle _spawn;
        _rocket setPosASL _spawn;

        // Velocity directly toward impact
        private _velDir = vectorNormalized (_impact vectorDiff _spawn);
        _rocket setVelocity (_velDir vectorMultiply 900);
